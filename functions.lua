...
MsgShow = 'يعني هه نص ونص' 
elseif NumMsg < 5000 then 
MsgShow = 'تفاعلك منتاز استمر' 
...
MsgShow = 'واضح ماعندك حياه غير التليغرام' 
elseif NumMsg > 150000 then 
MsgShow = 'ملك التليغرام' 
end
return MsgShow 
end
...
local message = '*⭐️│ المنشئين :*\n\n'
local monsha = redis:smembers(max..':MONSHA_BOT:'..msg.chat_id_)
if #monsha == 0 then 
message = message .."♦️│Not Creator ~⪼ لا يوجد منشئين !\n"
else
...
message = '📋*│* قائمة الأعضاء المميزين :\n'   
for k,v in pairs(list) do
local info = redis:hgetall(max..'username:'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.. '~⪼ '..(info.username or '')..' » ❪`' ..v.. '`❫ \n'
...
if not msg.SudoUser and not service then return '🚸│مسوي أبو ليلى ؟؟ ⚙️' end
...
function modrem(msg)
if not msg.SudoUser then return '🛠*│* مسوي أبو ليلى ؟؟ ⚙️'end
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك تقييد عمك\n🛠") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك تقييد عمك\n🛠") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك حظر عمك\n🛠") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك حظر عمك\n🛠") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك كتم عمك\n🛠") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك كتم عمك\n🛠") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك طرد عمك\n🛠") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك عمك\n🛠") 
elseif redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
...
return sendMsg(ChatID,MsgID,'♦️*╿*لا يمكنني فك تقييد العضو .\n🎟*╽*لانني لست مشرف في المجموعه \n ❕')    
end
end)
end,nil)
end
...
if cmd == "tqeed" then
if UserID == our_id then   
return sendMsg(ChatID,MsgID,"🏌‍♂*│*يطاوعك قلبك؟\n🛠") 
elseif UserID == SUDO_ID then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك طرد عمك\n🛠") 
elseif redis:sismember(max..':SUDO_BOT:',UserID) then 
return sendMsg(ChatID,MsgID,"🏌‍♂*│*لا يمكنك طرد عمك\n🛠") 
