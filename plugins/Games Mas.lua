--[[
الألعاب المعدلة لـ سورس بيسو 1.0
تم التعديل بواسطة ناشي @nashi080 
--]]
...
if SENDTEXT:find('عبدالرزاق حمدالله') then
...
elseif SENDTEXT:find('النصر') then
redis:set(max.."GAME:CHER"..msg.chat_id_,'النصر') 
elseif SENDTEXT:find('الانسان') then
redis:set(max.."GAME:CHER"..msg.chat_id_,'P8') 
...
katu = {
'جوز','طقاعه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
redis:set(max..':Set_Amthlh:'..msg.chat_id_,name)
name = string.gsub(name,'جوز','يعطي____للي ماعنده سنون')
name = string.gsub(name,'طقاعه','الي يسوق المطي يتحمل___')
name = string.gsub(name,'بيدك','اكل___محد يفيدك')
name = string.gsub(name,'الحافي','تجدي من___نعال')
name = string.gsub(name,'شقرا','مع الخيل يا___')
name = string.gsub(name,'نخله','الطول طول___والعقل عقل الصخلة')
...
if MsgText[1] == 'عكسسسا' or MsgText[1] == 'العكسسسا' or MsgText[1] == 'العكسسسا' then   
redis:set(max.."GAME:S"..msg.chat_id_,true) 
H1 = [[
✖️│عكس كلمه » {مافهمت} ⚜️
]]
...
sendMsg(msg.chat_id_,msg.id_,'*📮╿مبروك فزت وطلعت المحيبس باليد رقم { '..NUM..' }\n🎊╽لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *')   
